...
public class ExecutingAgentService {
	...
	public Flux<ServerSentEvent<DataMessage>> executing(GenerationSyntheticDataRequest request) {
		...
		String baseUserMessage = "[INPUT_USER_PROMPT: RAW_PROMPT] " + request.getPrompt();
	...
	private String getTokenMessage(ChatResponse chatResponse) {
	    if (chatResponse == null || chatResponse.getResult() == null) {
	        return "The sub-agent synthetic was unable to process the request.";
	    }
	    String content = chatResponse.getResult().getOutput().getText();
	    return (content != null && !content.isEmpty()) ? content : "Error: Sub-agent synthetic could not generate a response.";
	}
}
